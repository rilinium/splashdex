...
export async function renderFrog(canvas, colorId, patternId, genusId, chromaHue = 0) {
  ...
  try {
    ...
    const ctx = canvas.getContext('2d');
    // For glass: render full frog at full opacity onto a temp canvas,
    // then composite at 0.5 alpha so eyes/overlay remain crisp.
    ...
    if (isGlass) {
      ...
      _tintLayer(tCtx, genusImg, 0, 0, w, h, effPr, effPg, effPb);
...
export async function renderFrogOffscreen(colorId, patternId, genusId, size, chromaHue = 0) {
  ...
  if (isGlass) {
    ...
    _tintLayer(tCtx, baseImg,  0, 0, size, size, cr, cg, cb);
    _tintLayer(tCtx, genusImg, 0, 0, size, size, effPr, effPg, effPb);
    _multiplyOverlay(tCtx, ovImg, 0, 0, size, size);
